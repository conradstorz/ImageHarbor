...
def _merge_keyed_list(base: Any, new: Any, keys: tuple[str, ...], observed_at: str) -> list:
    ...
    Three policies operate on an existing entry's fields, matching the
    module's `_TIMESTAMP_FIELDS` vocabulary:
        ...
        if identity in index:
            ...
            for field, value in entry.items():
                ...
                if field == "first_seen":
                    ...
                    if field not in current or existing in (None, "", [], {}):
                        current[field] = value
                    elif value != existing:
                        ...
                        if isinstance(value, str) and isinstance(existing, str) and value < existing:
                            ...
                            loser = value
                        core = {"field": field, "value": loser}
                        if not any(_core(h) == core for h in history):
                ...
                if field not in current or existing in (None, "", [], {}):
                    ...
                    core = {"field": field, "value": existing}
                    if not any(_core(h) == core for h in history):
                        history.append({**core, "superseded_at": observed_at})
                    current[field] = value
            if history:
