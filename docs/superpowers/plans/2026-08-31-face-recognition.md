...
- **`replace_clusters` must preserve confirmed people.** Before deleting the old rows, build `{frozenset(face_ids): person_id}` for every cluster with a non-null `person_id`; after inserting the new rows, restore `person_id` onto any new cluster whose face set intersects an old confirmed one. A recluster is a machine operation and must never discard a human decision.
